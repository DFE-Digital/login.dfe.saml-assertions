...
const doesServiceMeetRequestCriteria = (service, req) => {
  ...
  if (
    ...
    service.organisationId.toLowerCase() !==
      req.params.organisationId.toLowerCase()
  ) {
    return false;
  }
...
const get = async (req, res) => {
  try {
    ...
    if (req.params.serviceId === "b45616a1-19a7-4a2e-966d-9e28c99bc6c6") {
      ...
      if (services) {
        const servicesMeetingCriteria = services.filter((s) =>
          doesServiceMeetRequestCriteria(s, req),
        );
        service =
          servicesMeetingCriteria && servicesMeetingCriteria.length > 0
            ? servicesMeetingCriteria[0]
            : undefined;
      }
      if (service) {
